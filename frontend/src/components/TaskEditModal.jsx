    ...
    import api from '../api';
    import { useAuth } from '../context/AuthContext'; // To get current user info for assignedTo dropdown

    const TaskEditModal = ({ task, isOpen, onClose, onTaskUpdated }) => {
        const { user } = useAuth(); // Assuming user has the ID and name
        ...
        // Update local state when 'task' prop changes (e.g., if re-opened for a different task)
        useEffect(() => {
            if (task) {
                setTitle(task.title);
        ...
        // Fetch organization members (simplified for now to just current user)
        useEffect(() => {
            if (user?.organization && isOpen) { // Only fetch when modal is open
                // In a real app, you'd fetch all organization members here
                // For now, we ensure the current user is an option
                setMembers([{ _id: user.id, name: user.name }]);
                // If the task is unassigned, and current user is the only option, default to them
                if (!task.assignedTo && !assignedTo && user.id) {
                    setAssignedTo(user.id);
                }
            }
        }, [user, isOpen, task, assignedTo]);
        ...
        return (
            <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
                <div className="relative p-8 bg-white w-full max-w-lg mx-auto rounded-lg shadow-xl animate-fade-in-up">
                    <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Edit Task</h3>
...
d</option>
                        ...
                        <div>
                            <label htmlFor="edit-dueDate" className="block text-sm font-medium text-gray-700">Due Date (Optional)</label>
                            <input
                                type="date"
                                id="edit-dueDate"
                                value={dueDate}
                        ...
                        <div>
                            <label htmlFor="edit-assignedTo" className="block text-sm font-medium text-gray-700">Assigned To (Optional)</label>
                            <select
                                id="edit-assignedTo"
                                value={assignedTo}
